class Admin::MiticWeb3ApplicantsController < ApplicationController
  before_action :admin_access

  def index
    order_by = params[:order_by].present? ? params[:order_by] : 'created_at'
    @applicants = MiticWeb3Applicant.order("#{order_by} DESC")

    if params[:status].present?
      @applicants = @applicants.where(status: MiticWeb3Applicant.statuses[params[:status]])
    end
    ...
    if params[:status].present? and params[:substate].present?
      @applicants = @applicants.find_applicant_by_substate(MiticWeb3Applicant, params[:status], params[:substate])
    end
  ...
  def show
    @applicant = MiticWeb3Applicant.find(params[:id])
  end
