...
RSpec.feature "mitic_applicant", type: :feature do
  let(:admin) { create(:admin_user) }
  let!(:applicant) { create(:mitic_applicant) }

  scenario "admin can lists and filters mitic applicants", js: true do
    login(admin)
    visit admin_mitic_applicants_path

    expect(page.all("table.table tr").count).to eq(MiticApplicant.count)
  ...
  scenario "admin can change the status of an applicant", js: true do
    ...
    expect(MiticApplicant.find(applicant.id).status).to eq("preselected")
  end
  ...
  scenario "admin can send an email to an applicant", js: true do
    ...
    visit admin_mitic_applicant_path(applicant)
