...
RSpec.describe UsersController, type: :controller do
  ...
  describe "GET #profile" do
    it "allows public access to public profiles" do
      get :profile, nickname: user_with_public_profile.nickname
      expect(response).to have_http_status(:ok)
    end

    it "doesn't allow access to private profiles" do
      expect { get :profile, nickname: user_with_private_profile.nickname }.to raise_error(ActionController::RoutingError)
    end

    it "should allow access to private profiles by admins" do
      controller.sign_in(create(:admin))

      get :profile, nickname: user_with_private_profile.nickname
      expect(response).to have_http_status(:ok)
    end
  end
  ...
  describe "GET #edit" do
    ...
    context "when signed in" do
      ...
      it "doesn't allow to edit other user" do
        ...
        expect { get :edit, id: other_user.id }.to raise_error(ActionController::RoutingError)
      end
    end
  ...
  describe "PATCH #update" do
    ...
    context "when not signed in" do
      it "redirects to login" do
        patch :update, id: user.id, user: atts
        expect(response).to redirect_to login_path
      end
    end
    ...
    context "when signed in" do
      ...
      context "with valid attributes" do
        ...
        it "doesn't update other user" do
          other_user = create(:user)
          expect { patch :update, id: other_user.id, user: atts }.to raise_error(ActionController::RoutingError)
        end
      end

      context "with invalid attributes" do
        it "renders the edit template" do
          patch :update, id: user.id, user: attributes_for(:user, nickname: "wro.ng")
          expect(response).to render_template :edit
        end
      end
    end
